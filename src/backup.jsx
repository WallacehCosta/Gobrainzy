...
const InteractiveQuiz = ({ messageIndex, message, updateProfile, userProfile }) => {
    ...
    const handleAnswer = (quizIndex, selectedIndex, correctIndex) => {
        ...
        // 1. Prepara o NOVO estado local e persistente
        ...
        // 2. Atualiza o estado local e pontuação total IMEDIATAMENTE
        setQuizStates(newStates);
        setTotalScore(prevScore => prevScore + scoreChange);
        
        // 3. Prepara a atualização do Firestore
        const updatedChatHistory = [...userProfile.chatHistory];
        
        // Atualiza a mensagem ESPECÍFICA no histórico com o novo quizStates
        updatedChatHistory[messageIndex] = {
            ...updatedChatHistory[messageIndex],
            suggestions: {
                ...updatedChatHistory[messageIndex].suggestions,
                quizStates: newStates // SALVA O NOVO ESTADO DO QUIZ NO FIREBASE
            }
        };

        // 4. Atualiza o perfil no Firestore (Pontos globais + Histórico com estado do quiz)
        const updates = { 
...
const ChatMessage = ({ message, messageIndex, updateProfile, userProfile, generateMentalMap, isLoading }) => {
    ...
    // Se for mensagem da IA
    return (
        <div className="flex justify-start mb-8">
            <div className="bg-white p-6 rounded-t-xl rounded-br-xl max-w-3xl shadow-xl border border-gray-100 space-y-6">
                <p className="font-semibold text-indigo-700">Tutor IA:</p>
                
                {/* Renderização da resposta com KaTeX */}
                ...
                {suggestions?.subject && (
                    <div className="flex justify-center">
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 max-w-md w-full">
                            <button
                                onClick={() => {
                                    console.log("Gerando mapa mental para:", suggestions.subject);
                                    generateMentalMap(suggestions.subject);
                                }}
                                disabled={isLoading}
                            ...
                            >
                                ...
                                {isLoading ? "Gerando Mapa Mental..." : "Gerar Mapa Mental deste Tópico"}
                            </button>
                            <p className="text-xs text-blue-600 mt-2 text-center">
                                Visualize os conceitos de forma organizada e hierárquica
                            </p>
                        </div>
                    </div>
                )}
                ...
                {suggestions && (suggestions.subject || suggestions.videoData || suggestions.dragAndDropTopic) && (
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            ...
                            {suggestions.subject && (
                                <div className="bg-green-50 p-4 rounded-xl shadow-sm border border-green-200">
                                    <p className="font-semibold text-green-700 flex items-center mb-2">
                                        <BookOpen className="w-4 h-4 mr-2" /> Matéria Principal
                                    </p>
                                    <p className="text-sm text-gray-800 font-medium">{suggestions.subject}</p>
                                </div>
                            )}
                            ...
                            {suggestions.videoData ? (
                                <div className="bg-red-50 p-4 rounded-xl shadow-sm border border-red-200 col-span-2">
                                    <p className="font-semibold text-red-700 flex items-center mb-2">
                                        <Video className="w-4 h-4 mr-2" /> Gostaria de Pesquisar Mais?
                                    </p>
                                    <div className="flex flex-col sm:flex-row gap-3 items-start">
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <p className="text-xs text-gray-600 mt-1">
                                                Método de Pesquisa: {suggestions.videoData.channel || 'Desconhecido'}
                                            </p>
                                            <p className="text-xs text-red-600 mt-2">
                                                Assunto: {suggestions.videoData.videoSubject}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-red-50 p-4 rounded-xl shadow-sm border border-red-200 col-span-2">
                                    ...
                                    {/* YouTube-style recommendation card */}
                                    <div className="flex flex-col sm:flex-row gap-3 items-start">
                                        <div className="flex-shrink-0 w-full sm:w-1/3 aspect-video bg-gray-200 rounded-lg overflow-hidden">
                                            <img
                                                src={`https://img.youtube.com/vi/${encodeURIComponent(suggestions.videoSubject).replace(/ /g, "_")}/hqdefault.jpg`}
                                                alt="Vídeo recomendado"
                                                className="w-full h-full object-cover"
                                                onError={(e) => { 
                                                    e.target.onerror = null;
                                                    e.target.src = 'https://placehold.co/1280x720/ef4444/ffffff?text=Recomendação+de+Vídeo'; 
                                                }}
                                            />
                                        </div>
                                        ...
                                        <div className="flex-grow">
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-base font-bold text-red-800 hover:underline line-clamp-2"
                                            >
                                                {suggestions.videoSubject.toUpperCase()}
                                            </a>
                                            <p className="text-sm text-gray-600 mt-1">
                                                Canal sugerido: <span className="font-medium">YouTube</span>
                                            </p>
                                            <p className="text-xs text-gray-500 mt-1">
                                                Clique para pesquisar vídeos sobre o tema.
                                            </p>
                                        </div>
                                    </div>
                                </div>
...
const generateMentalMap = async (topic) => {
  console.log("Iniciando geração de mapa mental para:", topic);

  setIsMindmapLoading(true);

  try {
    const prompt = `Com base no tópico "${topic}", gere um mapa mental em formato JSON com estrutura hierárquica. 
    Retorne APENAS um JSON válido com esta estrutura:
    {
      "name": "Tópico Principal",
      "children": [
        {"name": "Subtópico 1", "children": [...]},
        {"name": "Subtópico 2", "children": [...]}
      ]
    }
    Crie 3-4 subtópicos principais e 2-3 subitens para cada um.`;
    ...
    if (jsonMatch) {
      const mentalMapData = JSON.parse(jsonMatch[0]);
      console.log("Dados do mapa mental:", mentalMapData);
      setMentalMapTopics(mentalMapData.children || []);
      setShowMentalMap(true);
      console.log("Mapa mental gerado com sucesso!");
    } else {
      console.error("Não foi possível extrair JSON da resposta");
      showModal('Erro', 'Resposta da API inválida para mapa mental.');
    }
  } catch (error) {
